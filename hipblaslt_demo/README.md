...
- **`hipblaslt-bench`** on `PATH` after building hipBLASLt clients (see Step 0 in the markdown). Typical install output:  
  `…/hipblaslt/build/release/clients`.
- **GTest** (`libgtest-dev`, `libgmock-dev`) if CMake enables tests, or **`BUILD_TESTING=OFF`** in your own configure flags.
...
1. Open a Jupyter environment whose **kernel cwd** satisfies `find_repo_roots()` (see above).
